...
@section('content')
    ...
    @foreach ($packages as $package)
        <div class="package-products border rounded-3 p-3 pt-4 mb-4">
            ...
            <h5 class="fw-bold mb-0 my-2 mx-3 text-end">Dokopy: {{ number_format($package['total_price'], 2, ',', ' ') }} €</h5>
        </div>
    @endforeach
    ...
    <form method="POST" action="{{ route('cart-delivery.store') }}">
        ...
        @foreach($deliveryMethods as $value => $label)
            @php
                ...
                $onlyPersonal = $onlyPersonalAvailable ?? false;
                $locked = !$isPersonal && $onlyPersonal;
                $checked = $isPersonal && $onlyPersonal;
            @endphp
            <div class="form-check mb-2 {{ $locked ? 'text-muted' : '' }}">
                <input class="form-check-input"
                       ...
                       value="{{ $value }}"
                    {{ $locked ? 'disabled' : '' }}
                    {{ (old('deliveryMethod') ?? $selectedDelivery ?? ($checked ? $value : null)) === $value ? 'checked' : '' }}>
