...
@else
  ...
  <form method="POST" action="{{ route('cart.destroy') }}" id="removeSelectedForm">
    ...
    <div class="oznacenie d-flex justify-content-between align-items-center mb-3">
      ...
      <button type="submit"
              class="btn btn-link custom-button text-danger d-flex align-items-center p-0 text-decoration-none">
        Odstrániť označené
      ...
      <div class="package p-3 mb-4">
        ...
        @foreach($items as $item)
          @php
            ...
            $qty      = $item['quantity'];
            $prod     = $fp->product;
            $unitPrice = $fp->discount_percentage
                ? $fp->price_sell_quantity * (100 - $fp->discount_percentage)/100
                : $fp->price_sell_quantity;
          ...
          <div class="row cart-product align-items-center mb-3">
            ...
            <div class="col-auto">
              <img src="{{ asset($prod->image->path) }}"
                   alt="{{ $prod->image->name }}"
                   width="60" height="35">
            </div>
            <div class="col">
              <a href="{{ route('products.show',$fp->id) }}"
                 class="mb-1 fw-bold">{{ $prod->name }}</a>
            </div>
            <div class="col-auto mx-auto mt-1 gap-3 justify-content-center d-flex no-wrap">
              <span class="quantity me-3 d-flex align-items-center">
                <button type="button" class="quantity-button minus btn custom-button"
                        data-fpid="{{ $fp->id }}">−</button>
                ...
                <button type="button" class="quantity-button plus btn custom-button"
                        data-fpid="{{ $fp->id }}">+</button>
              </span>
              <span class="fw-bold inline-center line-total" data-fpid="{{ $fp->id }}">
                {{ $lineTotal }}
              </span>
              <form action="{{ route('cart-items.destroy', $fp->id) }}" method="POST">
                @csrf @method('DELETE')
                <button class="btn custom-button p-1" title="Odstrániť">
                  <span class="material-icons ms-2">delete</span>
                </button>
              </form>
            </div>
        ...
        @if(! $farm->delivery_available)
          <div class="text-end text-danger">Len osobné vyzdvihnutie</div>
        @elseif($farm->min_delivery_price !== null)
          <div class="text-end text-muted">
            Doprava od {{ number_format($farm->min_delivery_price,2) }} €
          </div>
        @endif
      </div>
    @endforeach
    ...
    @php
      $grand = collect($cartByFarm)
        ->flatten(1)
        ->sum(fn($item) => (
          ($item['fp']->discount_percentage
            ? $item['fp']->price_sell_quantity * (100-$item['fp']->discount_percentage)/100
            : $item['fp']->price_sell_quantity
          ) * $item['quantity']
        ));
    @endphp
    ...
    <div class="suhrn d-flex flex-column flex-sm-row align-items-sm-center justify-content-sm-between gap-3 mb-4">
      <div>
        <h4 class="fw-bold">
          Celkom na úhradu:
          <span id="cart-grand-total">{{ number_format($grand,2) }}</span> €
        </h4>
        <div class="text-muted fs-5">Cena bez DPH: …</div>
      </div>
      <a href="{{ route('cart-form.index') }}" class="btn custom-button px-4 py-2">
        Prejsť k údajom
      </a>
    </div>
